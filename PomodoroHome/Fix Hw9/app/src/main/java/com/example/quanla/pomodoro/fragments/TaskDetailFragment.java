...
public class TaskDetailFragment extends Fragment {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(((MainActivity)getActivity()).isOnline()){
            if (item.getItemId() == R.id.m_ok) {
                ...
                if (TaskFragment.viewClick == 0) {
                    ...
                    DbContext.instance.deleteATask(newTask);
        ...
        else Toast.makeText(getActivity(), "No internet No action", Toast.LENGTH_SHORT).show();
        return false;
    }
    ...
    public void addNewTask(Task task){
        ...
        String addNewTaskJson = (new Gson()).toJson(new AddTaskBodyJson(task.getColor(), false, null, null, uuid, task.getName(), task.getPayment()));
